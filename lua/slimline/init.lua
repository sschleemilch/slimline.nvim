...
---@alias Component {cfg: table, render: function}
---@alias Sep {left: string, right: string}
---@alias Highlights {primary: {text: string, sep: string, sep2sec: string}, secondary: {text: string, sep: string}}
...
local function get_component(component, position, direction)
    if type(component) == 'function' then
        return { cfg = nil, render = component }
    elseif type(component) == 'string' then
        ...
        if ok then
            ...
            if cfg.follow then
                cfg.hls = Slimline.highlights.hls.components[cfg.follow]
            else
                cfg.hls = Slimline.highlights.hls.components[component]
            end
            ...
            return {
                ...
                render = function(active)
                    if component == 'mode' or cfg.follow == 'mode' then
                        cfg.hls = Slimline.highlights.get_mode_hl(Slimline.get_mode().long)
                    end
                    return cmp.render(cfg, active)
                end,
            }
        else
            return {
